    ...
    public:
        void message_arrived(mqtt::const_message_ptr msg) override {
            ...
            if (topic == "station/c3/grid/raw") {
                if (mq_send(mqtt_grid_queue, msg_string.c_str(), msg_string.size(), 0) == -1) {
                ...
                if (mq_send(mqtt_weather_raw_queue, msg_string.c_str(), msg_string.size(), 0) == -1) {
                    std::cerr << "Error: Couldn't send msg to queue" << std::endl;
                    std::cerr << "mq_send errno: " << errno 
                            << " (" << strerror(errno) << ")" << std::endl;
                } else std::cout << "Recceived from " << topic <, std::endl;
            } else if (topic == "weather/avg") {
                if (mq_send(mqtt_weather_avg_queue, msg_string.c_str(), msg_string.size(), 0) == -1) {
                    std::cerr << "Error: Couldn't send msg to queue" << std::endl;
                    std::cerr << "mq_send errno: " << errno 
                            << " (" << strerror(errno) << ")" << std::endl;
                } else std::cout << "Recceived from " << topic <, std::endl;
...
int main() {
    ...
    attr_influx_queue.mq_maxmsg = 16;
